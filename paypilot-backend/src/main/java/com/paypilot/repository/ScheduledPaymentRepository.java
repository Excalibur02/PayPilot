...
public interface ScheduledPaymentRepository extends JpaRepository<ScheduledPayment, Long> {
    ...
    @Query("SELECT sp FROM ScheduledPayment sp WHERE sp.scheduledDate > CURRENT_DATE and sp.isPaid = false and sp.userId = :userId ORDER BY sp.scheduledDate")
    List<ScheduledPayment> getUpcomingPayments(@Param("userId") Long userId);

    @Query("SELECT sp FROM ScheduledPayment sp WHERE sp.scheduledDate < CURRENT_DATE or sp.isPaid = true and sp.userId = :userId ORDER BY sp.isPaid ASC, sp.scheduledDate ASC")
    List<ScheduledPayment> getPastPayments(@Param("userId") Long userId);

    // ✅ Custom query for sorted payments by user
    @Query("""
